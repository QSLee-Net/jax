...
def lower_xla_callable(fun: lu.WrappedFun, device, backend, name,
  ...
  if not jaxpr.eqns and not always_lower:
    return XlaComputation(
        ...
        in_avals=abstract_args, out_avals=out_avals, unordered_effects=[],
        ordered_effects=[], kept_var_idx=kept_var_idx, keepalive=None)
  ...
  return XlaComputation(
      ...
      unordered_effects=unordered_effects, ordered_effects=ordered_effects,
...
class XlaCompiledComputation(stages.Executable):
  ...
  def from_xla_computation(
    ...
    if ordered_effects or unordered_effects:
  ...
  def from_trivial_jaxpr(jaxpr, consts, device, in_avals, out_avals,
      unordered_effects, ordered_effects, kept_var_idx,
      keepalive: Optional[Any]) -> XlaCompiledComputation:
    assert keepalive is None
    result_handlers = map(partial(aval_to_result_handler, device), out_avals)
    unsafe_call = partial(_execute_trivial, jaxpr, device, consts,
                          out_avals, result_handlers, bool(unordered_effects),
                          ordered_effects, kept_var_idx)
